...
public class CreateField : MonoBehaviour {
	...
	public void SetTheDifficulty (int levelSelected) {
		...
		flagButton.blocksRaycasts=true;
	...
	public void GameOver(){
		...
		//turn off the flag button
		flagButton.alpha=0;
		flagButton.interactable=false;
		flagButton.blocksRaycasts=false;
	}
	...
	public void GameWon(){
		...
		FindObjectOfType<ScoreHandler>().CheckBestScore(difficultyLevel,score);
	...
	public void ToggleFlagging(){
		if(flagging==false){
			flagButton.GetComponent<Image>().color=Color.cyan;
			flagging=true;
		}else{
			flagging=false;
			flagButton.GetComponent<Image>().color=Color.white;
		}
	}
}
